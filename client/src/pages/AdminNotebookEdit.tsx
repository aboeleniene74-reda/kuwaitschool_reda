...
export default function AdminNotebookEdit() {
  ...
  const { data: notebook, isLoading: notebookLoading } = trpc.notebooks.getById.useQuery(
    { id: notebookId! },
    { enabled: !!notebookId }
  );
  ...
  // تحميل بيانات المذكرة عند الحصول عليها
  useEffect(() => {
    if (notebook) {
      setFormData({
        title: notebook.title,
        description: notebook.description || "",
        subjectId: notebook.subjectId.toString(),
        semesterId: notebook.semesterId.toString(),
        categoryId: notebook.categoryId.toString(),
  ...
  const updateNotebookMutation = trpc.notebooks.update.useMutation({
    onSuccess: () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!notebookId) {
      toast.error("معرف المذكرة غير صالح");
      return;
    }
    ...
    try {
      // رفع ملف PDF الكامل إذا تم اختيار ملف جديد
      if (pdfFile) {
      ...
      // رفع نسخة المعاينة إذا تم اختيار ملف جديد
      if (previewFile) {
  ...
  if (!notebook) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <Card className="max-w-md">
          <CardHeader>
            <CardTitle>المذكرة غير موجودة</CardTitle>
            <CardDescription>
              لم يتم العثور على المذكرة المطلوبة
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Link href="/admin/notebooks">
              <Button className="w-full">العودة لقائمة المذكرات</Button>
            </Link>
          </CardContent>
        </Card>
      </div>
        ...
        <div className="grid lg:grid-cols-4 gap-6">
          ...
          <main className="lg:col-span-3 space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-3xl font-bold">تعديل المذكرة</h2>
                <p className="text-muted-foreground">تعديل معلومات المذكرة الدراسية</p>
              </div>
              <Link href="/admin/notebooks">
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="space-y-4 pt-4 border-t">
                    <h3 className="font-semibold">رفع ملفات جديدة (اختياري)</h3>
                    <p className="text-sm text-muted-foreground">اترك الحقول فارغة إذا كنت لا تريد تغيير الملفات</p>
                    <div className="grid md:grid-cols-2 gap-4">
                      ...
                      {/* PDF File */}
                      <div className="space-y-2">
                        <Label htmlFor="pdfFile">ملف PDF الكامل</Label>
                        <div className="border-2 border-dashed rounded-lg p-6 text-center hover:border-primary transition-colors cursor-pointer"
                      ...
                      {/* Preview PDF File */}
                      <div className="space-y-2">
                        <Label htmlFor="previewFile">نسخة المعاينة</Label>
                  ...
                  <div className="flex gap-3 pt-4">
                    <Button
                      type="submit"
                      disabled={updateNotebookMutation.isPending || uploading}
                      className="flex-1"
                    >
                      {uploading ? "جاري رفع الملفات..." : updateNotebookMutation.isPending ? "جاري الحفظ..." : "حفظ التعديلات"}
                    </Button>
                    <Link href="/admin/notebooks">
